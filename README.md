...
1. Download the latest release: [pocket-showroom-core-v3.3.2.zip](https://github.com/evolution301/pocket-showroom-core/releases/download/v3.3.2/pocket-showroom-core.zip)
...
### v3.3.2
- **Feature**: Added a smart "Scroll to Top" behavior. When you click a category filter and you are currently scrolled deep into the product grid, the page will automatically and smoothly scroll back up to the filter bar, allowing you to view the new category's products from the very beginning.

### v3.3.1
- **Feature**: Added a smart scroll listener that seamlessly hides the Website Theme Header (like navigation bars) when you scroll down into the product grid. This maximizes screen space for viewing products. Scanning upwards instantly reveals the header again.

### v3.3.0
- **Feature**: Removed the desktop left sidebar entirely in favor of a cleaner, 100% width product grid.
- **Feature**: The top category filter pills now stick to the top of the screen (`position: sticky`) as you scroll down, allowing easy filtering from anywhere in the gallery without needing a persistent sidebar.

### v3.2.2
- **Feature**: Upgraded Category Sidebar and Filter buttons to use an AJAX-driven Auto-Load mechanic instead of hiding/showing local DOM nodes.
- **Fix**: Clicking a category now automatically queries the server for products inside that category, removing the need to manually click "Load More" to discover them.
- **Fix**: The "Load More" button now correctly respects the currently active category filter instead of reverting to 'all'.
...
- **New**: Smart Quotation Cart & PDF Request
- **New**: Sales Rep Avatar & Floating Contact Widget
- **New**: Fair Mode (Printable QR Code Labels)
- **New**: Private Mode (Password Protection)
- **UI**: Complete Design System Overhaul (Royal Blue Theme)

### v2.0.2 (2026-02-15)
- **Fix**: Resolved "Settings page access denied" error by correcting the admin menu link URL.
